  ...
  <v-data-table :headers="headers" :items="wom.items" dense  class="elevation-1"  :search="search"
    ...
    <template v-slot:top >
        ...
        <v-toolbar flat dark dense color="blue darken-4">
            <v-toolbar-title>WO Material List</v-toolbar-title>
    ...
    <template v-slot:item.action="{ item }"><!--1=qd,2-inpr,3-complt----->
       ...
       <v-btn ripple small :loading="loading" color="teal " rounded dark   @click.prevent="getonewomaterial(item)"><v-icon >mdi-mouse</v-icon></v-btn>
      ...
      editedIndex: -1, sawflags:[],// inputRules:[v=>v.length>=3||'Min len is 3 chars'],
              ...
              // { text: 'Select', value: 'action', sortable: false , width:"1%"},
               { text: 'Details', value: 'action', sortable: false , width:"1%"},
                { text: 'CustomerName', align: 'left',  value: 'CustomerName', width:"1%"},
               { text: 'ReservationQuantity', align: 'left',  value: 'ReservationQuantity', width:"1%"},
               { text: 'DemandSourceTypeName', align: 'left',  value: 'DemandSourceTypeName', width:"1%"},
               { text: 'ReservationUnitOfMeasure', value: 'ReservationUnitOfMeasure', sortable: false , width:"1%"},
               { text: 'SupplySourceHeaderId', align: 'left',  value: 'SupplySourceHeaderId', width:"1%"},
             ...
             // { text: 'QuantityPERProduct', align: 'left',  value: 'QuantityPERProduct', width:"1%"},
              { text: 'DueDate', align: 'left',  value: 'DueDate', width:"1%"},
          ...
          computed: { 
          ...mapState({
             wom:state => state.saw.getworeservation.data,
        }),
          },
  created(){ 
        },
    ...
    getonewomaterial(x){
              console.log('getonereservation-',x)
               this.$router.push({   name: 'woreservationdetails',params: {data1: x }  
                               });
    }
      }
